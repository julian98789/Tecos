...
const Table = ({ tables, updateTableState }) => {
    ...
    const { logged, user, getUserData } = useStore();
    ...
    const handleClick = async () => {
        if (!ocupada && (role !== 'cajero' && role !== 'admin')) {
    ...
    return (
        <div onClick={handleClick} className={`w-[150px] h-[150px] flex justify-center items-center rounded-lg ${ocupada ? 'bg-zinc-600 cursor-default' : 'bg-zinc-200 hover:transition transition-transform hover:scale-105 duration-300 shadow-2xl cursor-pointer'}`}>
